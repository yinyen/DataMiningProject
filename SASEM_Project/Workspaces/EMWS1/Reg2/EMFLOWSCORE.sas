...
*** Generate dummy variables for audience_score_positive ;
drop _Y ;
label F_audience_score_positive = 'From: audience_score_positive' ;
length F_audience_score_positive $ 5;
F_audience_score_positive = put( audience_score_positive , $5. );
%DMNORMIP( F_audience_score_positive )
if missing( audience_score_positive ) then do;
   _Y = .;
end;
else do;
   if F_audience_score_positive = 'FALSE'  then do;
      _Y = 1;
   end;
   else if F_audience_score_positive = 'TRUE'  then do;
      _Y = 0;
   end;
   else do;
      _Y = .;
...
'Residual: audience_score_positive=False' ;
   R_audience_score_positiveTrue = - _P0;
   R_audience_score_positiveFalse = - _P1;
   select( _Y );
      when (0)  R_audience_score_positiveTrue = R_audience_score_positiveTrue
         + 1;
      when (1)  R_audience_score_positiveFalse = 
